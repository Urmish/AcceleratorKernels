...
public:
    ...
    enum { DEFAULT_NLEVELS=4 };

    CV_WRAP HOGDescriptor2() : winSize(64,128), blockSize(16,16), blockStride(8,8),
        cellSize(8,8), nbins(9), derivAperture(1), winSigma(-1),
        histogramNormType(HOGDescriptor2::L2Hys), L2HysThreshold(0.2), gammaCorrection(true),
        nlevels(HOGDescriptor2::DEFAULT_NLEVELS)
    {}

    CV_WRAP HOGDescriptor2(Size _winSize, Size _blockSize, Size _blockStride,
                  Size _cellSize, int _nbins, int _derivAperture=1, double _winSigma=-1,
                  int _histogramNormType=HOGDescriptor2::L2Hys,
                  double _L2HysThreshold=0.2, bool _gammaCorrection=false,
                  int _nlevels=HOGDescriptor2::DEFAULT_NLEVELS)
    ...
    CV_WRAP HOGDescriptor2(const String& filename)
    {
        load(filename);
    }

    HOGDescriptor2(const HOGDescriptor2& d)
    {
        d.copyTo(*this);
    }

    virtual ~HOGDescriptor2() {}
    ...
    virtual void copyTo(HOGDescriptor2& c) const;
   ...
   //                                                    std::vector<DetectionROI2>& locations,
...
class CV_EXPORTS SimilarRects2
{
public:
    SimilarRects2(double _eps) : eps(_eps) {}
...
//             "  HOGDescriptor2::hog.setSVMDetector(HOGDescriptor2::getDefaultPeopleDetector());\n"
//             "Usage:\n"
//             "./peopledetect (<image_filename> | <image_list>.txt)\n\n");
// }
size_t HOGDescriptor2::getDescriptorSize() const
{
...
double HOGDescriptor2::getWinSigma() const
{
    return winSigma >= 0 ? winSigma : (blockSize.width + blockSize.height)/8.;
}

bool HOGDescriptor2::checkDetectorSize() const
{
...
void HOGDescriptor2::setSVMDetector(InputArray _svmDetector)
{
...
bool HOGDescriptor2::read(FileNode& obj)
{
    if( !obj.isMap() )
        return false;
...
void HOGDescriptor2::write(FileStorage& fs, const String& objName) const
{
    if( !objName.empty() )
        fs << objName;
...
bool HOGDescriptor2::load(const String& filename, const String& objname)
{
...
void HOGDescriptor2::save(const String& filename, const String& objName) const
{
    FileStorage fs(filename, FileStorage::WRITE);
    write(fs, !objName.empty() ? objName : FileStorage::getDefaultObjectName(filename));
}

void HOGDescriptor2::copyTo(HOGDescriptor2& c) const
{
    c.winSize = winSize;
...
struct HOGCache2
{
    ...
    HOGCache2();
    HOGCache2(const HOGDescriptor2* descriptor,
        const Mat& img, Size paddingTL, Size paddingBR,
        bool useCache, Size cacheStride);
    virtual ~HOGCache2() {};
    virtual void init(const HOGDescriptor2* descriptor,
        const Mat& img, Size paddingTL, Size paddingBR,
        bool useCache, Size cacheStride);
    ...
    Mat grad, qangle;
    const HOGDescriptor2* descriptor;
};
...
HOGCache2::HOGCache2(const HOGDescriptor2* _descriptor,
...
void HOGCache2::init(const HOGDescriptor2* _descriptor,
        const Mat& _img, Size _paddingTL, Size _paddingBR,
        bool _useCache, Size _cacheStride)
{
    ...
    //   1. loop over windows is unrolled in the HOGDescriptor2::{compute|detect} methods;
...
const float* HOGCache2::getBlock(Point pt, float* buf)
{
    float* blockHist = buf;
    assert(descriptor != 0);
...
//ACCPOT
void HOGCache2::normalizeBlockHistogram(float* _hist) const
{
    float* hist = &_hist[0];
...
Size HOGCache2::windowsInImage(Size imageSize, Size winStride) const
{
    return Size((imageSize.width - winSize.width)/winStride.width + 1,
                (imageSize.height - winSize.height)/winStride.height + 1);
}

Rect HOGCache2::getWindow(Size imageSize, Size winStride, int idx) const
{
...
void HOGDescriptor2::compute(const Mat& img, vector<float>& descriptors,
                            ...
                            const vector<Point>& locations) const
{
    ...
    for( size_t i = 0; i < nwindows; i++ )
    {
        ...
        {
            const HOGCache2::BlockData& bj = blockData[j];
...
void HOGDescriptor2::detect(const Mat& img,
    ...
    Size winStride, Size padding, const vector<Point>& locations) const
{
    ...
    for( size_t i = 0; i < nwindows; i++ )
    {
        ...
        int j, k;
        for( j = 0; j < nblocks; j++, svmVec += blockHistogramSize ) //nblocks is 105
        {
            const HOGCache2::BlockData& bj = blockData[j];
            Point pt = pt0 + bj.imgOffset;
...
void HOGDescriptor2::detect(const Mat& img, vector<Point>& hits, double hitThreshold,
                           Size winStride, Size padding, const vector<Point>& locations) const
{
...
{
public:
    HOGInvoker( const HOGDescriptor2* _hog, const Mat& _img,
...
void HOGDescriptor2::detectMultiScale(
    ...
    double scale0, double finalThreshold, bool useMeanshiftGrouping) const
{
   ...
   // new ;
    printf("Invoking HOGInvoker with levelscale %d\n",(int)levelScale.size());
    HOGInvoker HOGInvokerObject(this, img, hitThreshold, winStride, padding, &levelScale[0], &allCandidates, &mtx, &tempWeights, &tempScales);
...
//                HOGInvoker(this, img, hitThreshold, winStride, padding, &levelScale[0], &allCandidates, &mtx, &tempWeights, &tempScales));
    ...
    int my = (int)levelScale.size() ;
    for (int i=0; i<my; i++) //ACCPOT - Could be parallelized - Each core would run on one image
    {
	HOGInvokerObject.my_operator(Range(i,i+1));
    }
//(Range(0, (int)levelScale.size()), HOGInvokerObject);
...
void HOGDescriptor2::detectMultiScale(const Mat& img, vector<Rect>& foundLocations,
                                     double hitThreshold, Size winStride, Size padding,
                                     double scale0, double finalThreshold, bool useMeanshiftGrouping) const
{
...
vector<float> HOGDescriptor2::getDaimlerPeopleDetector()
{
    static const float detector[] = {
...
//{
//public:
//       HOGConfInvoker( const HOGDescriptor2* _hog, const Mat& _img,
//                               double _hitThreshold, Size _padding,
//                               std::vector<DetectionROI2>* locs,
...
void HOGDescriptor2::detectROI(const cv::Mat& img, const vector<cv::Point> &locations,
                                       CV_OUT std::vector<cv::Point>& foundLocations, CV_OUT std::vector<double>& confidences,
                                       ...
                                       cv::Size padding) const
{
   ...
   // HOGCache2 cache(this, img, padding, padding, nwindows == 0, cacheStride);
   HOGCache2 cache(this, img, padding, padding, true, cacheStride);
   if( !nwindows )
           nwindows = cache.windowsInImage(paddedImgSize, winStride).area();
...
//void HOGDescriptor2::detectMultiScaleROI(const cv::Mat& img,
//                                         CV_OUT std::vector<cv::Rect>& foundLocations,
//                                         std::vector<DetectionROI2>& locations,
//                                                           double hitThreshold,
...
void HOGDescriptor2::readALTModel(std::string modelfile)
{
...
void HOGDescriptor2::groupRectangles(vector<cv::Rect>& rectList, vector<double>& weights, int groupThreshold, double eps) const
{
    ...
    vector<int> labels;
    int nclasses = partition(rectList, labels, SimilarRects2(eps));
...
int main(int argc, char** argv)
{
    ...
    HOGDescriptor2 hog;
    hog.setSVMDetector(HOGDescriptor2::getDefaultPeopleDetector());
